...
describe("[Core] Entity", () => {
	describe("Constructor Guard", () => {
		...
		test("throws DomainError when props is a Date", () => {
			expect(() => User.init(new Date() as unknown as UserProps)).toThrow(DomainError);
		});

		test("throws DomainError when props is a primitive", () => {
			expect(() => User.init("string" as unknown as UserProps)).toThrow(DomainError);
		});
	});
	...
	describe("Static Factories", () => {
		describe("Entity.create()", () => {
			test("returns success when props are valid", () => {
				const result = User.create({ name: "Ash", profile: { city: "Pallet", level: 1 } });
			...
			test("returns error when props fail subclass validation", () => {
				const result = User.create({ name: "", profile: { city: "Pallet", level: 1 } });
		...
		describe("Entity.init()", () => {
			test("returns instance directly when props are valid", () => {
				const user = User.init({ name: "Ash", profile: { city: "Pallet", level: 1 } });
	...
	describe("Instance Methods", () => {
		describe("entity.id", () => {
			test("auto-generates a new UID when no id is provided", () => {
				const user = User.create({ name: "Ash", profile: { city: "Pallet", level: 1 } }).value();
			...
			test("restores a string ID from props and marks it as NOT new", () => {
				const user = User.create({ id: "user-1", name: "Misty", profile: { city: "Cerulean", level: 2 } }).value();
			...
			test("restores a numeric ID from props and casts it to string", () => {
				const user = User.create({ id: 42, name: "Brock", profile: { city: "Pewter", level: 3 } }).value();
			...
			test("restores a UID instance from props", () => {
				...
				const user = User.create({ id: uid, name: "Gary", profile: { city: "Pallet", level: 5 } }).value();

				expect(user.id.value()).toBe("uid-abc");
			});

			test("get('id') returns the resolved string value, not a UID object", () => {
				const user = User.create({ id: "user-99", name: "Red", profile: { city: "Viridian", level: 10 } }).value();
		...
		describe("entity.get()", () => {
			...
			test("returns implicit props: id, createdAt, updatedAt", () => {
				const user = User.create({ id: "u-1", name: "Ash", profile: { city: "Pallet", level: 1 } }).value();
		...
		describe("entity.change() / entity.set().to()", () => {
			...
			test("change() with a failing validation function throws DomainError and leaves props unchanged", () => {
				...
				expect(() => user.change("name", "", (v) => v.trim().length > 0)).toThrow(DomainError);
				expect(user.get("name")).toBe("Ash");
			});
		...
		describe("entity.clone()", () => {
			...
			test("inherits the original id when no id override is provided", () => {
				const user = User.init({ id: "u-1", name: "Ash", profile: { city: "Pallet", level: 1 } });
				const cloned = user.clone();
		...
		describe("entity.isEqual()", () => {
			test("two entities with the same id and same props are equal", () => {
				const a = User.init({ id: "same", name: "Ash", profile: { city: "Pallet", level: 1 } });
				const b = User.init({ id: "same", name: "Ash", profile: { city: "Pallet", level: 1 } });
			...
			test("two entities with the same id but different props are not equal", () => {
				const a = User.init({ id: "same", name: "Ash", profile: { city: "Pallet", level: 1 } });
				const b = User.init({ id: "same", name: "Ash", profile: { city: "Viridian", level: 1 } });

				expect(a.isEqual(b)).toBe(false);
			});

			test("two entities with different ids are not equal, even with the same props", () => {
				const a = User.init({ id: "id-a", name: "Ash", profile: { city: "Pallet", level: 1 } });
				const b = User.init({ id: "id-b", name: "Ash", profile: { city: "Pallet", level: 1 } });

				expect(a.isEqual(b)).toBe(false);
			});

			test("entities of different subclasses with the same id are NOT equal (critical DDD rule)", () => {
				const user = User.init({ id: "shared-id", name: "Ash", profile: { city: "Pallet", level: 1 } });
				const admin = Admin.init({ id: "shared-id", name: "Ash", profile: { city: "Pallet", level: 1 } });
		...
		describe("entity.hashCode()", () => {
			...
			test("two different instances produce different hash codes", () => {
				const a = User.create({ name: "Ash", profile: { city: "Pallet", level: 1 } }).value();
				const b = User.create({ name: "Misty", profile: { city: "Cerulean", level: 2 } }).value();
		...
		describe("entity.toObject()", () => {
			...
			test("recursively serializes nested entities, VOs, and UID props", () => {
				const friend = User.init({ id: "friend", name: "Brock", profile: { city: "Pewter", level: 3 } });
